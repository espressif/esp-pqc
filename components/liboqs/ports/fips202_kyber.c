 ...
 *
 * Taken from the Kyber reference implementation, ref/fips202.c:
 * https://github.com/pq-crystals/kyber/blob/4768bd37c02f9c40a46cb49d4d1f4d5e612bb882/ref/fips202.c
 *
 * The API, round constants, permutation and SHAKE/SHA3 entry points are
 * unchanged, so this file matches the reference output bit for bit. The
 * Espressif changes tune the buffer handling for 32-bit ESP32 targets; each one
 * is marked with an "Espressif change" comment at its site.
 */
...
static uint64_t load64(const uint8_t x[8]) {
  /* Espressif change: memcpy instead of the upstream byte loop. ESP32 targets
   * are little-endian, so this compiles to a plain word load. */
  uint64_t r;
  memcpy(&r, x, 8);
  return r;
}
...
static void store64(uint8_t x[8], uint64_t u) {
  /* Espressif change: memcpy instead of the upstream byte loop (little-endian).
   */
  memcpy(x, &u, 8);
}
...
/* Espressif addition: optional Keccak permutation counter for benchmark
 * analysis, enabled by LIBOQS_DEBUG_INSTRUMENTATION (Kconfig, default off).
 * One count per KeccakF1600_StatePermute() call: squeezeblocks does one per
 * block, absorb can add more. */
#if defined(MLD_KECCAK_COUNT_VERIFY)
static uint64_t s_keccak_permute_count = 0;
static uint64_t s_squeeze_blocks_requested =
    0; /* total nblocks passed to shake128_squeezeblocks */
#endif
...
static void keccak_init(uint64_t s[25]) {
  /* Espressif change: memset instead of the upstream lane loop. */
  memset(s, 0, 25 * sizeof(uint64_t));
}

/* Espressif addition: accessors for the permutation counters above. */
#if defined(MLD_KECCAK_COUNT_VERIFY)
...
uint64_t mld_keccak_count_get(void) { return s_keccak_permute_count; }
...
static unsigned int keccak_absorb(uint64_t s[25], unsigned int pos,
  ...
  while (pos + inlen >= r) {
    if (pos == 0) {
      /* Espressif change: whole lane-aligned blocks are absorbed 64 bits at a
       * time; upstream walks them byte by byte. */
      for (i = 0; i < r / 8; i++)
        s[i] ^= load64(in + 8 * i);
      in += r;
      inlen -= r;
...
static unsigned int keccak_squeeze(uint8_t *out, size_t outlen, uint64_t s[25],
  ...
  /* Espressif change: fast path, output full blocks with a single memcpy
   * instead of the upstream byte loop. */
  ...
  /* Espressif change: remainder, full blocks with memcpy when pos==0, else
   * byte-by-byte */
  while (outlen) {
    if (pos == r) {
      KeccakF1600_StatePermute(s);
...
static void keccak_absorb_once(uint64_t s[25], unsigned int r,
  ...
  while (inlen >= r) {
    /* Espressif change: lane-oriented absorb of whole blocks (upstream is
     * byte-wise). */
    for (i = 0; i < r / 8; i++)
      s[i] ^= load64(in + 8 * i);
    in += r;
  ...
  /* Espressif change: tail, lane-oriented for complete 8-byte chunks, then byte
   * for remainder */
...
static void keccak_squeezeblocks(uint8_t *out, size_t nblocks, uint64_t s[25],
  ...
  while (nblocks) {
    KeccakF1600_StatePermute(s);
    /* Espressif change: one memcpy per block instead of the upstream byte loop.
     */
    memcpy(out, s, r);
    out += r;
    nblocks -= 1;
...
/* Espressif addition: instrumentation only, no effect on the output. */
#if defined(MLD_KECCAK_COUNT_VERIFY)
